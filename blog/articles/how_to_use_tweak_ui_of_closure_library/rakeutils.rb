...
module RakeUtils
  ...
  class CssOptimizer
    ...
    def optimize(*paths)
      ...
      css = map_urls(css)      unless @css_url_map.empty?
    ...
    def expand_import(path)
      ...
      css = css.gsub(/^@import(?:\s+url\()?\s*[\"\']?([^\"\'\)]+)[\"\']?\s*\)?([^;]*);/u) do
        url, media = $1.strip, $2.strip
        url = map_url(url)
        if /^https?:\/\//u === url
          "@import url(#{url}) #{media};"
        else
          if /^\/+(.+)$/u === url
            sub_css = expand_import($1)
          else
    ...
    def map_url(url)
      @css_url_map.each do |pattern, replace|
        return url.gsub(pattern, replace) if pattern === url
      end
      url
    end
